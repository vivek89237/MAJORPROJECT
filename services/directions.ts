...
export async function getDirections(from:[number, number], to:[number, number], type:string) {
  const response = await fetch(
    `${BASE_URL}/directions/v5/mapbox/${type}/${from[0]},${from[1]};${to[0]},${to[1]}?alternatives=false&annotations=distance%2Cduration&continue_straight=true&geometries=geojson&overview=full&steps=false&access_token=${process.env.EXPO_PUBLIC_MAPBOX_KEY}`
  );
